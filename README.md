...
After logging in the **Transition** screen helps to distinguish between seller and buyer. If the logged in user is a buyer, he gets to the [Buyer homepage](#availability-map) (upper screen) otherwise (he is a seller) he gets to the [Seller homepage](#openbooked-offers) (lower screen).

![image](https://user-images.githubusercontent.com/53949039/177571500-d8ae0e75-b362-497c-a081-b4feb735ee65.png)

### *C. Navigation bar*

The **Navigation bar** is almost the same for both seller & buyer.

- The left icon ("Map" or "Home") it directs you to the respective homescreen ([Buyer](#availability-map) or [Seller](#openbooked-offers)).
- "Invoices" is not impemented yet.
- "My Score" directs you to the [Scoring list](#scoring-list) (only implemented for the buyer yet). 
- "Profile" links to respective profile of the user ([Seller profile](#seller-profile) or [Buyer profile](#buyer-profile)).
...
- Invoices
- Forecast of energy production (e.g. by using weather forecast and the size of the producers solar panel)
- Scoring list for the seller
- FAQ
- ....
